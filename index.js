...
export async function xink(xink_config) {
  ...
  return {
    ...
    transform(src, id) {
      if (
        id === join(cwd, entrypoint) &&
        serve_options &&
        new Set(['bun', 'deno']).has(runtime)
      ) {
        /* Built entrypoint should include <runtime>.serve */
        ...
        s.replace(
          /export default {\n(.*)\n};/s,
          runtime.charAt(0).toLocaleUpperCase() +
            runtime.slice(1) +
            `.serve({\n$1,${o.toString()}\n});`,
        )
        return s.toString()
    ...
    async configureServer(server) {
      ...
      server.middlewares.use(async (req, res) => {
        try {
          ...
          const api = await server.ssrLoadModule(join(cwd, entrypoint))
          const base = `${
