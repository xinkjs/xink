...
export const resolve = async (event) => {
  ...
  const validation = async (validators, type) => {
    ...
    for (let i = 0; i < validators.length; i++) {
      ...
      if (validator_type === 'json') {
        ...
        event.valid.json = schema ? await standardValidator(validator, json_body) : validator(json_body)
      }
      ...
      if (validator_type === 'form') {
        ...
        event.valid.form = schema ? await standardValidator(validator, form_values) : validator(form_values)
      }

      if (validator_type === 'params') {
        event.valid.params = schema ? await standardValidator(validator, event.params) : validator(event.params)
      }
      ...
      if (validator_type === 'query') {
        ...
        event.valid.query = schema ? await standardValidator(validator, query_obj) : validator(query_obj)
      }
    }
  }
