  ...
  if (event.store.validators[event.request.method]) {
    ...
    for (let i = 0; i < validators.length; i++) {
      ...
      if (validator_type === 'json') {
        ...
        event.valid.json = v.parse ? v.parse(json_body) : v.schema.parse(json_body)
      }
      ...
      if (validator_type === 'form') {
        ...
        event.valid.form = v.parse ? v.parse(form_values) : v.schema.parse(form_values)
      }
      ...
      if (validator_type === 'query') {
        ...
        event.valid.query = v.parse ? v.parse(query_values) : v.schema.parse(query_values)
      }
    }
  }
