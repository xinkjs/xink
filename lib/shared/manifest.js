...
export const createManifest = async (runner, config, mode) => {
  ...
  const { middleware_dir, out_dir, params_dir, routes_dir } = config
  const outdir = mode === 'development' ? '.xink' : out_dir
  const manifest = {
    routes: {},
    params: {},
    middleware: null
  ...
  const readParamsDir = async () => {
    ...
    for (const path of readFiles(params_dir)) {
      ...
      const build_params_path = join(out_dir, params_dir, `${type}.js`)
  ...
  const initMiddleware = async () => {
    for (const path of readFiles(middleware_dir, { exact: true, filename: 'middleware' })) {
      ...
      const build_middleware_path = join(out_dir, middleware_dir, `middleware.js`)
  ...
  for (let p of readFiles(routes_dir, { filename: 'route' })) {
    ...
    const safe_path = path.replace(/=/g, '_').replace(/:([\w.~-]+)/g, '_$1_')
    const build_endpoint_path = safe_path === '/' ? join(out_dir, routes_dir, `route.js`) : join(out_dir, routes_dir, safe_path.slice(1), `route.js`)
  ...
  const routes_manifest_path = join(cwd, outdir, `manifest.json`)
  const routes_manifest_dir = join(cwd, outdir)
