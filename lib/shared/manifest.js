...
export const createManifest = async (runner, config, mode) => {
  ...
  const readParamsDir = async () => {
    ...
    for (const path of readFiles(params_dir)) {
      ...
      const build_params_path = `${out_dir}/${params_dir}/${type}.js`
  ...
  const initMiddleware = async () => {
    for (const path of readFiles(middleware_dir, { exact: true, filename: 'middleware' })) {
      ...
      const build_middleware_path = `${out_dir}/${middleware_dir}/middleware.js`
  ...
  for (let p of readFiles(routes_dir, { filename: 'route' })) {
    ...
    const build_endpoint_path = safe_path === '/' ? `${out_dir}/${routes_dir}/route.js` : `${out_dir}/${routes_dir}/${safe_path.slice(1)}/route.js`
  ...
  const routes_manifest_path = `${cwd}/${outdir}/manifest.json`
  const routes_manifest_dir = `${cwd}/${outdir}`
