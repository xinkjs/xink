...
export const validateConfig = (config) => {
  ...
  if (config.csrf?.check && typeof config.csrf.check !== 'boolean')
    throw new Error(`csrf.check must be a boolean, but "${config.csrf.check}" is a ${typeof config.csrf.check}.`)

  if (config.csrf?.origins) {
    if (!Array.isArray(config.csrf.origins))
      throw new Error(`csrf.origins must be an array, but found "${config.csrf.origins}".`)

    config.csrf.origins.forEach(o => {
      if (typeof o !== 'string')
        throw new Error(`csrf.origins must be an array of strings, but found "${o}" is not.`)
    })
  }
